...
namespace Bot.Builder.Community.Twitter.Webhooks.Authentication
{
    ...
    public class AuthHeaderBuilder
    {
        ...
        /// <returns></returns>
        public static string Build(TwitterAuthContext authContext, HttpMethod method, string requestUrl)
        {
            ...
            {
                throw new TwitterException("Invalid Tweety Auth Context.");
            }
            ...
            var epochTimeStamp = (DateTime.UtcNow - new DateTime(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc)).TotalSeconds;
            ...
            var baseString = string.Join("&", signatureParams.OrderBy(kpv => kpv.Key).Select(kpv => $"{kpv.Key}={kpv.Value}"));

            var resourceUrl = requestUrl.Contains("?") ? requestUrl.Substring(0, requestUrl.IndexOf("?")) : requestUrl;
            baseString = string.Concat(method.Method.ToUpper(), "&", Uri.EscapeDataString(resourceUrl), "&", Uri.EscapeDataString(baseString));

            var oauthSignatureKey = string.Concat(Uri.EscapeDataString(authContext.ConsumerSecret), "&", Uri.EscapeDataString(authContext.AccessSecret));
